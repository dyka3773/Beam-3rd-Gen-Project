...
async def run_sensors_cycle(starting_time: float):
    ...
    while (time.perf_counter() - starting_time < TimelineEnum.SODS_OFF.value):
        ...
        await data_manager.save_sensor_data(temp_1, temp_2, press_1, press_2)
        logging.debug(f'Temperature 1: {temp_1} - Temperature 2: {temp_2} - Pressure 1: {press_1} - Pressure 2: {press_2}')
